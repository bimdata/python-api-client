...
def parse_commit_message(message):
    ...
    if config.get('semantic_release', 'minor_tag') in message.lower():
        level = 'feature'
        level_bump = 2
        subject = message.replace(config.get('semantic_release', 'minor_tag'.format(level)), '')

    elif config.get('semantic_release', 'fix_tag') in message.lower():
        level = 'fix'
        level_bump = 1
        subject = message.replace(config.get('semantic_release', 'fix_tag'.format(level)), '')

    elif config.get('semantic_release', 'major_tag') in message.lower():
        level = 'breaking'
        level_bump = 3
